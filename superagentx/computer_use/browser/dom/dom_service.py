...
def _parse_node(
    ...
    if 'viewport' in node_data:
        viewport_info = ViewportInfo(
            width=node_data.get('viewport')('width'),
            height=node_data.get('viewport')('height'),
        )
...
async def _construct_dom_tree(
    ...
    for id, node_data in js_node_map.items():
        ...
        if isinstance(node, DOMElementNode) and node.highlight_index:
...
class DomService:
    def __init__(self, page: 'Page'):
        ...
        self.js_code = (files('superagentx.computer_use.browser.dom') / 'dom_builder.js').read_text()
    ...
    async def _build_dom_tree(
        ...
        return await _construct_dom_tree(eval_page)
