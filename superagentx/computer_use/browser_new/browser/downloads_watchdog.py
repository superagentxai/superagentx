...
class DownloadsWatchdog(BaseWatchdog):
	...
	def _track_download(self, file_path: str) -> None:
		...
		try:
			...
			if path.exists():
				...
				from superagentx.computer_use.browser_new.browser.events import FileDownloadedEvent
	...
	async def _handle_cdp_download(
		...
		downloads_dir = Path(
			...
			or f'{tempfile.gettempdir()}/superagentx.computer_use.browser_new_downloads.{str(self.browser_session.id)[-4:]}'
		)
		try:
