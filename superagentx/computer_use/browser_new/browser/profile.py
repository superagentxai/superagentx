...
from superagentx.computer_use.browser_new.config import CONFIG
from superagentx.computer_use.browser_new.observability import observe_debug
from superagentx.computer_use.browser_new.utils import _log_pretty_path, logger
...
# Using constants from central location in superagentx.computer_use.browser_new.config
...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	proxy: ProxySettings | None = Field(
		default=None,
		description='Proxy settings. Use superagentx.computer_use.browser_new.browser.profile.ProxySettings(server, bypass, username, password)',
	)
	...
	def warn_user_data_dir_non_default_version(self) -> Self:
		...
		if self.user_data_dir == CONFIG.superagentx.computer_use.browser_new_DEFAULT_USER_DATA_DIR and is_not_using_default_chromium:
			alternate_name = (
			...
			self.user_data_dir = CONFIG.superagentx.computer_use.browser_new_DEFAULT_USER_DATA_DIR.parent / f'default-{alternate_name}'
		return self
	...
	def _ensure_default_extensions_downloaded(self) -> list[str]:
		...
		cache_dir = CONFIG.superagentx.computer_use.browser_new_EXTENSIONS_DIR
