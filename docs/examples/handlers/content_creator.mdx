...
async def content_creator_init():
...
async def content_creator(instruction):
    handler_object = await content_creator_init()
    result = await handler_object.text_creation(instruction=instruction)
