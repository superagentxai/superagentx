...
(Large Language Model) client using OpenAI's API. for more detail about `AgentXPipe` refer <a href="pipe" target='_blank'>here</a>.
```python
...
#### 3. Define Memory
This line enables memory functionality in the pipeline by configuring a Memory object that utilizes the previously created llm_client for storing and retrieving context.
```python
    # Enable Memory
