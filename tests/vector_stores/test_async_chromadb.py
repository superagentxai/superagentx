...
'''
 Run Pytest:
   ...
   1. pytest --log-cli-level=INFO tests/vector_stores/test_chromadb.py::TestChroma::test_chromadb_client
   2. pytest --log-cli-level=INFO tests/vector_stores/test_chromadb.py::TestChroma::test_collection_exists
   3. pytest --log-cli-level=INFO tests/vector_stores/test_chromadb.py::TestChroma::test_document_insert
   4. pytest --log-cli-level=INFO tests/vector_stores/test_chromadb.py::TestChroma::test_search
   5. pytest --log-cli-level=INFO tests/vector_stores/test_chromadb.py::TestChroma::test_update
   6. pytest --log-cli-level=INFO tests/vector_stores/test_chromadb.py::TestChroma::test_delete_collection
'''
...
class TestChroma:
    ...
    async def test_collection_exists(self, chromadb_client_init: VectorStore):
        if await chromadb_client_init.aexists():
            assert True
        else:
            assert False
    ...
    async def test_document_insert(self, chromadb_client_init: VectorStore):
        ...
        try:
            _insert = await chromadb_client_init.ainsert(texts=documents, ids=["id1", "id2"])
    ...
    async def test_search(self, chromadb_client_init: VectorStore):
        _search = await chromadb_client_init.asearch(
    ...
    async def test_update(self, chromadb_client_init: VectorStore):
        try:
            _update = await chromadb_client_init.aupdate(
                vector_id="id1",
                embeddings=None,
                payload=None
            )
    ...
    async def test_delete_collection(self, chromadb_client_init: VectorStore):
        try:
            _delete = await chromadb_client_init.adelete_collection()
