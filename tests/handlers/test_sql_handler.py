...
class TestSql:
    ...
    def test_create_table(self, sql_client_init: SQLHandler):
        stmt = "CREATE TABLE test5 (x int, y int)"
        res = sql_client_init.handle(action="CREATE_TABLE",
                                     stmt=stmt
                                     )
    ...
    def test_insert_table(self, sql_client_init: SQLHandler):
        stmt = "INSERT INTO test2 (x, y) VALUES (:x, :y)"
        values = [{'x': 1, 'y': 2}, {'x': 2, 'y': 4}]
        res = sql_client_init.handle(
            action="INSERT",
            stmt=stmt,
            values=values
        )
    ...
    def test_select_table(self, sql_client_init: SQLHandler):
        res = sql_client_init.handle(
            action="SELECT",
    ...
    def test_drop_table(self, sql_client_init: SQLHandler):
        res = sql_client_init.handle(
            action="DROP_TABLE",
            stmt="DROP TABLE test3"
        )
        res_dict = res.context.__dict__
        assert isinstance(res, CursorResult)